...
class _AdminAnalyticsScreenState extends State<AdminAnalyticsScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final approvedReviews = allReviews.where((r) => r.reviewStatus == ReviewStatus.approved).toList();
    final pendingReviews = allReviews.where((r) => r.reviewStatus == ReviewStatus.pending).toList();
    final averageRating = approvedReviews.isEmpty 
        ? 0 
        : approvedReviews.map((r) => r.rating).reduce((a, b) => a + b) / approvedReviews.length;
    
    // Get top rated listings with proper typing
    ...
    // Sort with safe comparison
    topListings.sort((a, b) {
      final aRating = a['avgRating'] as double;
      final bRating = b['avgRating'] as double;
              ...
              child: Column(
                ...
                children: [
                  ...
                  ...topListings.take(5).map((listing) {
                    final listingTitle = listing['listingTitle'] as String;
                    final avgRating = listing['avgRating'] as double;
                    final reviewCount = listing['reviewCount'] as int;
                    return ListTile(
                      leading: Container(
                        width: 40,
                        height: 40,
